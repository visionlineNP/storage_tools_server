...
class Database:
    ...
    def _add_name(self, table:str, name:str, description:str):
        with self.mutex:
            name_to_index = {name: i for i, (name, _) in enumerate(self.database[table])}
    ...
    def add_run(self, project, site, robot_name, run_name, duration, total_size):
        with self.mutex:
            name_to_index = { (entry[0], entry[1], entry[2], entry[3]): i for i, entry in enumerate(self.database["runs"])}
        ...
        with self.mutex:                    
            self.database["runs"].append(( project_idx, site_idx, robot_idx, run_name, duration, total_size ))
    ...
    def add_data(self, entry:dict):
        ...
        with self.mutex:
            name_to_index = { f"{item['reldir']}/{item['basename']}":i for i, item in enumerate(self.database["data"]) }
    ...
    def get_data(self):
        ...
        for entry in self.database["data"]:
            ...
            rtn[project][robot][run].append({
                "datetime":date, "relpath": relpath, "basename": basename, "size": size, "site": site, "run_name": run, "datatype": datatype})
        return rtn 
    ...
    def get_run_stats(self):
        ...
        for entry in self.database["data"]:
            ...
            stats[project][ymd][run] = stats[project][ymd].get(run, {
                "total_size": 0, 
                "count": 0,
                "start_datetime": None,
                "end_datetime": None,
    ...
    def estimate_runs(self):
        entry_set = [i for i, entry in enumerate(self.database["data"]) if entry["datatype"].lower() == ".mcap" or entry["datatype"].lower() == ".bag" ]
            ...
            for run_id, group in enumerate(groups):
                ...
                for entry in self.database["data"]:
                    if entry["project"] != project:
                        continue 
                    if  start_time <= entry["start_datetime"] <= end_time or start_time <= entry["end_datetime"] <= end_time:                        
