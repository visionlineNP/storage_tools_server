...
class FileSpeedEstimate:
    ...
    def update(self, amount):
        ...
        if current_time > (self.update_rate + self.update_time):
            ...
            self.transfer_speed_bytes_per_second = (0.90 * float(self.phase_copied) / time_delta) + (0.10 *self.transfer_speed_bytes_per_second)
            remaining_size = self.file_size - self.transfered_size
            self.seconds_remaining = float(remaining_size) / self.transfer_speed_bytes_per_second
    ...
    def getText(self):
        ...
        speed = humanfriendly.format_size(int(self.transfer_speed_bytes_per_second)) + "/S"
